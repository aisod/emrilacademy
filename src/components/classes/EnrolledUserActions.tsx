...
export function EnrolledUserActions({
  class_,
  isSessionActive
}: EnrolledUserActionsProps) {
  const {
    handleJoinLiveClass
  } = useLiveSession();
  return <>
      {class_.class_type === "live" && isSessionActive && <Button onClick={() => handleJoinLiveClass(class_.id)} variant="default" className="w-full group text-sky-500">
